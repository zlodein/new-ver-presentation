...
function getContentSettings(c: unknown): ContentSettings | undefined {
  ...
  const template = typeof s.template === 'string' ? s.template : undefined
  ...
  if (
    template == null && exportEnabled == null && fontFamily == null && imageBorderRadius == null && imageFrame == null &&
...
export async function presentationRoutes(app: FastifyInstance) {
  ...
  /** Восстановление мягко удалённой презентации (id в теле, как при удалении). */
  app.post<{ Body: { id?: string } }>(
    '/api/presentations/restore',
    { preHandler: [app.authenticate] },
    async (req: FastifyRequest<{ Body: { id?: string } }>, reply: FastifyReply) => {
      const userId = getUserId(req)
      const id = req.body?.id != null ? String(req.body.id).trim() : ''
