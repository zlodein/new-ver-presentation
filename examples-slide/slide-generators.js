// Словарь русских названий типов слайдов
                ...
                <div class="booklet-content booklet-main">
                    <div class="booklet-main__wrap">
                        ...
                        <div class="booklet-main__content">
                            ...
                            <!-- Все валюты (отображаются по умолчанию в редакторе) -->
                            <div class="all-currencies editor-currencies" id="editor-all-currencies-${index}" style="margin-top: 10px;">
                            ...
                            <div style="margin-top: 10px; display: flex; align-items: center; justify-content: flex-end;">
                                <label style="display: flex; align-items: center; gap: 8px; font-size: 13px; color: #555; cursor: pointer; padding: 6px 10px; background: #f8f9fa; border-radius: 6px; border: 1px solid #e0e0e0;">
                                    <input type="checkbox" 
                                           id="show-currencies-${index}"
                                           data-slide="${index}"
                                           ${slide.show_currencies ? 'checked' : ''}
                                           onchange="toggleCurrenciesVisibility(${index})"
            ...
            <div class="booklet-page__inner">
                <div class="booklet-content booklet-char">
                    <div class="booklet-char__wrap">
                        <div contenteditable="true" class="booklet-char__title editor-field" data-field="title" data-slide="${index}">${slide.title || 'ХАРАКТЕРИСТИКИ'}</div>
...
// Генерация слайда "Планировка"
function generateLayoutSlide(slide, index) {
    const image = slide.image || '';
                ...
                <div class="booklet-content booklet-layout">
                    <div class="booklet-layout__wrap">
                        <div class="booklet-layout__title-wrapper">
                            <div contenteditable="true" class="booklet-layout__title editor-field" data-field="title" data-slide="${index}">${slide.title || 'ПЛАНИРОВКА'}</div>
                        </div>
                        <div class="booklet-layout__img">
                            <input type="file" accept="image/*" class="hide-input" id="layout-img-${index}" data-slide="${index}" data-upload-type="layout-image">
                            <button class="btn-edit-bg btn-edit-bg--center" onclick="document.getElementById('layout-img-${index}').click()"></button>
                            ${image ? `<img src="${image}" alt="">` : ''}
                        </div>
                    </div>
                </div>
                    ...
                    <div class="booklet-info__wrap">
                        <div class="booklet-info__block booklet-info__content">
                            ...
                            <div class="booklet-info__text">
                                <div class="text-generator-wrapper">
                                    <button class="btn-generate-text" onclick="generateTextWithAI(${index}, 'description')" title="Сгенерировать текст с помощью AI">
                                        <i class="fas fa-magic"></i> Сгенерировать текст
                                    </button>
                    ...
                    <div class="booklet-stroen__wrap">
                        <div class="booklet-stroen__block booklet-stroen__content">
                            ...
                            <div class="booklet-stroen__text">
                                <div class="text-generator-wrapper">
                                    <button class="btn-generate-text" onclick="generateTextWithAI(${index}, 'infrastructure')" title="Сгенерировать текст с помощью AI">
                                        <i class="fas fa-magic"></i> Сгенерировать текст
                                    </button>
...
function generateLocationSlide(slide, index) {
                ...
                <div class="booklet-content booklet-map">
                    <div class="booklet-map__wrap">
                        ...
                        <div class="booklet-map__img">
                            <div id="yandex-map-${index}" class="yandex-map-container" style="width: 100%; height: 100%; min-height: 300px; border-radius: 8px;"></div>
                        </div>
                        <div class="booklet-map__content">
                            ...
                            <div class="booklet-map__info">
                                ...
                                <div class="booklet-map__text">
                                    <input 
                                        type="text" 
                                        class="editor-field location-address-input" 
                                        id="location-address-${index}"
                                        data-field="location_address" 
                                        data-slide="${index}"
                                        placeholder="Введите адрес для поиска"
                                        value="${address}"
                                        style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; font-family: inherit; font-size: inherit;"
                                    />
                                    <input type="hidden" id="location-lat-${index}" value="${lat}" data-field="location_lat" data-slide="${index}">
                                    <input type="hidden" id="location-lng-${index}" value="${lng}" data-field="location_lng" data-slide="${index}">
                                    <small style="display: block; margin-top: 4px; color: #666; font-size: 11px;">Начните вводить адрес для получения подсказок</small>
                                    
                                    <button 
                                        type="button"
                                        class="btn btn-sm btn-outline-primary mt-2" 
                                        onclick="findNearestMetro(${index})"
                                        style="padding: 5px 12px; font-size: 12px;">
                                        <i class="fas fa-subway"></i> Найти ближайшее метро
                                    </button>
                                </div>
                                
                                <div id="metro-info-${index}" class="metro-info mt-3" style="display: ${metroStations.length > 0 ? 'block' : 'none'};">
                                    <div style="background: #e3f2fd; border-left: 4px solid #2196f3; padding: 12px; border-radius: 4px;">
                                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                            <strong style="color: #1976d2; font-size: 13px;">
                                                <i class="fas fa-subway"></i> Ближайшее метро:
                                            </strong>
                                            <label style="display: flex; align-items: center; font-size: 11px; color: #666; font-weight: normal; margin: 0; cursor: pointer;">
                                                <input type="checkbox" 
                                                       id="show-metro-${index}"
                                                       data-slide="${index}"
                                                       data-field="show_metro"
                                                       ${showMetro ? 'checked' : ''}
                                                       onchange="toggleMetroVisibility(${index})"
                                                       style="margin-right: 4px;">
                                                Показывать в презентации
                                            </label>
                                        </div>
                                        <div id="metro-list-${index}" style="margin-top: 8px;">
                                            ${renderMetroList(metroStations)}
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    setTimeout(() => {
        const mapContainer = document.getElementById(`yandex-map-${index}`);
        const addressInput = document.getElementById(`location-address-${index}`);
        const latInput = document.getElementById(`location-lat-${index}`);
        const lngInput = document.getElementById(`location-lng-${index}`);
        
        if (!mapContainer || !addressInput) {
            console.warn(`Карта или поле ввода не найдены для слайда ${index}`);
            return;
        }
        ...
        yandexMaps.initEditorMap(
            ...
            console.log(`Карта успешно инициализирована для слайда ${index}`);
        }).catch(error => {
        ...
        const locationUpdateHandler = (e) => {
            const currentAddress = document.getElementById(`location-address-${index}`);
            if (currentAddress && (document.activeElement === currentAddress || e.detail.address === currentAddress.value)) {
                ...
                if (slides[index]) {
                    slides[index].location_lat = e.detail.lat;
                    slides[index].location_lng = e.detail.lng;
                    slides[index].location_address = e.detail.address;
                    ...
                    console.log(`Местоположение обновлено для слайда ${index}:`, e.detail);
                }
            }
        };
        ...
        addressInput.addEventListener('blur', function() {
            if (slides[index]) {
                slides[index].location_address = this.value;
                hasUnsavedChanges = true;
                if (typeof triggerAutoSave === 'function') {
                    triggerAutoSave();
                }
        ...
        if (!window.yandexMapHandlers) {
            window.yandexMapHandlers = {};
        }
        window.yandexMapHandlers[`slide-${index}`] = {
...
// Функция отрисовки списка метро
function renderMetroList(stations) {
    if (!stations || stations.length === 0) {
        return '<p style="color: #666; font-size: 12px; margin: 0;">Станции не найдены</p>';
    }
    
    let html = '<ul style="list-style: none; padding: 0; margin: 0; font-size: 12px;">';
    stations.forEach(station => {
        html += `
            <li style="margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px solid rgba(0,0,0,0.1);">
                <strong style="color: #1976d2;">${station.name}</strong><br>
                <span style="color: #666;">
                    <i class="fas fa-walking" style="width: 14px;"></i> ${station.walk_time_text} пешком
                    <span style="margin: 0 5px;">•</span>
                    <i class="fas fa-car" style="width: 14px;"></i> ${station.drive_time_text} на авто
                    <span style="margin: 0 5px;">•</span>
...
async function findNearestMetro(slideIndex) {
    const slide = slides[slideIndex];
    const lat = slide.location_lat;
    const lng = slide.location_lng;
    
    if (!lat || !lng) {
        showNotification('Сначала укажите адрес на карте', 'error');
        return;
    }
    
    // Показываем индикатор загрузки
    const metroInfo = document.getElementById(`metro-info-${slideIndex}`);
    const metroList = document.getElementById(`metro-list-${slideIndex}`);
    ...
    try {
        ...
        if (data.success && data.stations) {
            // Сохраняем станции в слайд
            slides[slideIndex].metro_stations = data.stations;
            // По умолчанию показываем метро
            if (slides[slideIndex].show_metro === undefined) {
                slides[slideIndex].show_metro = true;
            }
            
            // Отображаем результаты
            metroList.innerHTML = renderMetroList(data.stations);
            
            if (data.stations.length > 0) {
                showNotification(`Найдено станций: ${data.stations.length}`, 'success');
            } else {
                metroList.innerHTML = '<p style="color: #666; font-size: 12px; margin: 0;">Станции метро не найдены поблизости</p>';
            }
        ...
        metroList.innerHTML = '<p style="color: #dc3545; font-size: 12px; margin: 0;"><i class="fas fa-exclamation-triangle"></i> Ошибка при поиске станций</p>';
        showNotification('Ошибка при поиске станций метро', 'error');
    }
}

// Переключение видимости метро в презентации (десктопная версия)
function toggleMetroVisibility(slideIndex) {
    const checkbox = document.getElementById(`show-metro-${slideIndex}`);
    if (checkbox) {
        slides[slideIndex].show_metro = checkbox.checked;
        hasUnsavedChanges = true;
        triggerAutoSave();
        showNotification(
            checkbox.checked ? 'Метро будет показано в презентации' : 'Метро будет скрыто в презентации',
            'success'
        );
    }
}
                ...
                <div class="editor__inner">
                    <div class="editor__inner-toolbar">
                        ...
                        <div class="slide-type-badge" style="background: #666; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px; margin-top: 5px;">
                            ${slideTypeNames[slide.type] || slide.type}
                        </div>
                    </div>
