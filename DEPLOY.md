...
**Важно:** для продакшен-сборки фронта `VITE_API_URL` должен быть **пустым**, чтобы запросы шли на относительный `/api` (ваш домен). Иначе в браузере будут запросы на localhost и подсказки/метро/генерация не заработают. Перед сборкой скопируйте пример и соберите: `cp .env.production.example .env.production` (или убедитесь, что в `.env.production` строка `VITE_API_URL=` пустая), затем `npm run build:prod` и деплой.
