          ...
          <div class="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
            <div class="mb-5 sm:mb-8">
              <h1 class="mb-2 font-semibold text-gray-800 text-title-sm dark:text-white/90 sm:text-title-md">
                {{ hasToken ? 'Новый пароль' : 'Забыли пароль?' }}
              </h1>
              <p class="text-sm text-gray-500 dark:text-gray-400">
                {{
                  hasToken
                    ? 'Введите новый пароль для вашего аккаунта.'
                    : 'Введите email, привязанный к аккаунту, и мы отправим ссылку для восстановления.'
                }}
              </p>
            </div>
            ...
            <form v-if="!hasToken" @submit.prevent="requestReset" class="space-y-5">
              ...
              <p class="text-sm text-center text-gray-700 dark:text-gray-400">
                Вспомнили пароль?
                <router-link to="/signin" class="text-brand-500 hover:text-brand-600 dark:text-brand-400">Нажмите здесь</router-link>
              </p>
            </form>
