  ...
  <PublicLayout>
    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:gap-6">
      ...
      >
        ...
        <button
          type="button"
          :disabled="isLoggedIn && !canChooseTestDrive"
...
function redirectToAuth() {
  const redirect = encodeURIComponent(route.fullPath || '/tariffs')
  router.push(`/signin?redirect=${redirect}`)
}

function onChooseTestDrive() {
  if (!isLoggedIn.value) {
    redirectToAuth()
    return
  }
  chooseTariff('test_drive')
}

function onChooseExpert() {
  if (!isLoggedIn.value) {
    redirectToAuth()
    return
  }
