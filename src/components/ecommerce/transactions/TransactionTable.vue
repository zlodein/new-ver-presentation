      ...
      <div class="flex gap-3.5">
        <div class="hidden flex-col gap-3 sm:flex sm:flex-row sm:items-center">
          ...
          <div class="hidden lg:block relative">
            <select
              class="dark:bg-dark-900 shadow-theme-xs appearance-none focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
            >
          ...
          <div>
            <button
              class="hover:text-dark-900 shadow-theme-xs relative flex h-11 items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-3 text-gray-700 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="20"
      ...
      <table class="w-full table-auto">
          ...
          <tr class="border-b border-gray-200 dark:divide-gray-800 dark:border-gray-800">
            <th class="p-4">
              <div class="flex w-full items-center gap-3">
                ...
                <p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Order ID</p>
              </div>
            </th>
            <th class="p-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400">
              <div class="flex cursor-pointer items-center gap-3" @click="sortBy('customer')">
                ...
                <span class="flex flex-col gap-0.5">
                  <svg
                    :class="{
                      'text-gray-800 dark:text-gray-400': sort.key === 'customer' && sort.asc,
                      'text-gray-300': !(sort.key === 'customer' && sort.asc),
                    }"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
            ...
            <th class="p-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400">
              <div class="flex cursor-pointer items-center gap-3" @click="sortBy('email')">
                <p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Email</p>
                <span class="flex flex-col gap-0.5">
                  <svg
                    :class="{
                      'text-gray-800 dark:text-gray-400': sort.key === 'email' && sort.asc,
                      'text-gray-300': !(sort.key === 'email' && sort.asc),
                    }"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
            ...
            <th class="p-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400">
              <div class="flex cursor-pointer items-center gap-3" @click="sortBy('amount')">
                ...
                <span class="flex flex-col gap-0.5">
                  <svg
                    :class="{
                      'text-gray-800 dark:text-gray-400': sort.key === 'amount' && sort.asc,
                      'text-gray-300': !(sort.key === 'amount' && sort.asc),
                    }"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
        ...
        <tbody class="divide-x divide-y divide-gray-200 dark:divide-gray-800">
          ...
          >
              ...
              <div class="group flex items-center gap-3">
                ...
                >
                  <span class="relative">
                    ...
                    <span
                      :class="{
                        'border-brand-500 bg-brand-500': selected.includes(row.id),
                      ...
                      class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                    >
                      <span :class="{ 'opacity-0': !selected.includes(row.id) }">
                        <svg
                          width="12"
                          height="12"
                          viewBox="0 0 12 12"
            ...
            <td class="p-4 whitespace-nowrap">
              <p class="text-sm text-gray-700 dark:text-gray-400">{{ row.amountDisplay }}</p>
            </td>
            <td class="p-4 whitespace-nowrap">
              <p class="text-sm text-gray-700 dark:text-gray-400">{{ row.dueDate }}</p>
            </td>
            <td class="p-4 whitespace-nowrap">
              <span
              ...
              >
              <span
                class="bg-warning-50 dark:bg-warning-500/15 text-warning-700 dark:text-warning-500 text-theme-xs rounded-full px-2 py-0.5 font-medium"
    ...
    <div class="border-t border-gray-200 px-5 py-4 dark:border-gray-800">
      <div class="flex justify-center pb-4 sm:hidden">
        <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">
          Showing <span class="text-gray-800 dark:text-white/90">{{ startEntry }}</span> to
          <span class="text-gray-800 dark:text-white/90">{{ endEntry }}</span> of
          ...
          <ul class="hidden items-center gap-0.5 sm:flex">
            <li v-for="n in totalPages" :key="n">
              <a
                ...
                :class="{
                  'bg-brand-500 text-white': page === n,
...
const sort = ref({ key: 'customer', asc: true })
const page = ref(1)
const perPage = 10
const openDropdown = ref(null)

const totalPages = computed(() => Math.ceil(rows.value.length / perPage) || 1)
...
const paginatedRows = computed(() => {
  return sortedRows.value.slice((page.value - 1) * perPage, page.value * perPage)
})
...
function toggleSelectAll() {
  if (selectAll.value) {
    selected.value = paginatedRows.value.map((t) => t.id)
  } else {
    selected.value = []
  }
}
...
function updateSelectAll() {
  selectAll.value = paginatedRows.value.every((row) => selected.value.includes(row.id))
}
