...
<template>
  <div v-if="enabled" class="figures-panel mt-2 rounded-xl border border-gray-200 bg-white/60 p-3 dark:border-gray-700 dark:bg-gray-900/30">
    ...
    <div v-if="open" class="mt-3">
            ...
            >
              <span class="inline-flex h-6 w-6 items-center justify-center rounded bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
                <svg viewBox="0 0 100 100" class="h-4 w-4" xmlns="http://www.w3.org/2000/svg">
                  ...
                  />
                  <ellipse v-else-if="(f.geometry as any)?.kind === 'ellipse'" cx="50" cy="50" rx="50" ry="50" fill="currentColor" class="text-brand-600" />
                  ...
                  <polygon
                    ...
                    points="50,8 61,35 90,35 66,52 75,78 50,62 25,78 34,52 10,35 39,35"
                    fill="currentColor"
                    class="text-brand-600"
                  />
                  <polygon v-else-if="(f.geometry as any)?.kind === 'polygon'" :points="(f.geometry as any)?.points?.map((p: number[]) => `${p[0]},${p[1]}`).join(' ')"
                    fill="currentColor" class="text-brand-600" />
                  <line v-else-if="['line','connector'].includes((f.geometry as any)?.kind)" x1="10" y1="50" x2="90" y2="50" stroke="currentColor" stroke-width="8" stroke-linecap="round" />
                </svg>
              </span>
      ...
      <div v-if="selectedInstance" class="mt-3 rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
        <div class="flex items-center justify-between gap-2">
          ...
          <button type="button" class="rounded-lg border border-red-200 bg-red-50 px-2 py-1 text-xs text-red-700 hover:bg-red-100 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-300" @click="removeSelected">
            Удалить
          </button>
        </div>
        ...
        <div class="mt-3 space-y-2">
          ...
          <div v-else-if="(selectedInstance.style as any)?.fill?.type === 'linear'">
            <div class="grid grid-cols-2 gap-2">
              <div>
                ...
                <input type="color" :value="(selectedInstance.style as any)?.fill?.from ?? '#60a5fa'"
                  class="h-9 w-full rounded-lg border border-gray-300 bg-white p-1 dark:border-gray-700 dark:bg-gray-800"
              ...
              <div>
                ...
                <input type="color" :value="(selectedInstance.style as any)?.fill?.to ?? '#1d4ed8'"
                  class="h-9 w-full rounded-lg border border-gray-300 bg-white p-1 dark:border-gray-700 dark:bg-gray-800"
            ...
            <div>
              ...
              <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                {{ (selectedInstance.style as any)?.fill?.angle ?? 45 }} deg
              </div>
            </div>
          </div>

          <div v-else class="text-xs text-gray-500 dark:text-gray-400">
            Заливка отключена для выбранной фигуры.
          </div>
          ...
          <div class="pt-2">
            <label class="flex items-center gap-2 text-xs font-medium text-gray-700 dark:text-gray-300">
              <input
                type="checkbox"
                :checked="Boolean((selectedInstance.style as any)?.stroke?.enabled)"
                class="rounded border-gray-300 text-brand-600"
                @change="setStrokeEnabled(($event.target as HTMLInputElement).checked)"
              />
              Границы (stroke)
            </label>
          </div>
          ...
          <div v-if="Boolean((selectedInstance.style as any)?.stroke?.enabled)" class="space-y-2">
            <div>
              ...
              <input
                type="color"
                :value="(selectedInstance.style as any)?.stroke?.color ?? '#111827'"
                class="h-9 w-full rounded-lg border border-gray-300 bg-white p-1 dark:border-gray-700 dark:bg-gray-800"
            ...
            <div>
              ...
              <input
                ...
                max="12"
                step="0.5"
                :value="Number((selectedInstance.style as any)?.stroke?.width ?? 2)"
                class="w-full"
                @input="setStrokeWidth(Number(($event.target as HTMLInputElement).value))"
              />
              <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                {{ Number((selectedInstance.style as any)?.stroke?.width ?? 2).toFixed(1) }} px
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2">
              <div>
                ...
                <select
                  class="h-9 w-full rounded-lg border border-gray-300 bg-white px-2 text-xs text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
                  :value="(selectedInstance.style as any)?.stroke?.dash ?? 'solid'"
                  @change="setStrokeDash(($event.target as HTMLSelectElement).value as any)"
                >
          ...
          <div class="pt-2">
            ...
            <input
              ...
              max="360"
              step="1"
              :value="Number((selectedInstance.rotation ?? 0))"
              class="w-full"
              @input="setRotation(Number(($event.target as HTMLInputElement).value))"
            />
            <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
              {{ Number(selectedInstance.rotation ?? 0) }} deg
            </div>
          </div>
          ...
          <div class="pt-2">
            ...
            <div class="grid grid-cols-2 gap-2">
              <button type="button" class="h-8 rounded-lg border border-gray-200 bg-white px-2 text-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700" @click="moveLayer(-1)">
                На зад
              </button>
              <button type="button" class="h-8 rounded-lg border border-gray-200 bg-white px-2 text-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700" @click="moveLayer(1)">
                На перед
              </button>
              <button type="button" class="h-8 rounded-lg border border-gray-200 bg-white px-2 text-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700" @click="sendToBack">
                К началу
              </button>
              <button type="button" class="h-8 rounded-lg border border-gray-200 bg-white px-2 text-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700" @click="bringToFront">
                К концу
              </button>
            </div>
          </div>
          ...
          <div v-if="Boolean((selectedInstance.style as any)?.shadow?.enabled)">
            <div class="grid grid-cols-2 gap-2">
              <div>
                ...
                <input type="color" :value="(selectedInstance.style as any)?.shadow?.color ?? '#000000'"
                  class="h-9 w-full rounded-lg border border-gray-300 bg-white p-1 dark:border-gray-700 dark:bg-gray-800"
              ...
              <div>
                ...
                <input type="range" min="0" max="1" step="0.01" :value="Number((selectedInstance.style as any)?.shadow?.opacity ?? 0.25)"
                  class="w-full"
                  @input="setShadowOpacity(Number(($event.target as HTMLInputElement).value))" />
                <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">{{ Number((selectedInstance.style as any)?.shadow?.opacity ?? 0.25).toFixed(2) }}</div>
              </div>
            </div>

            <div class="mt-2">
              <label class="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Размытие</label>
              <input type="range" min="0" max="24" step="1" :value="Number((selectedInstance.style as any)?.shadow?.blur ?? 6)"
                class="w-full"
                @input="setShadowBlur(Number(($event.target as HTMLInputElement).value))" />
            </div>

            <div class="mt-2 grid grid-cols-2 gap-2">
              <div>
                <label class="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Смещение X</label>
                <input type="range" min="-20" max="20" step="1" :value="Number((selectedInstance.style as any)?.shadow?.offsetX ?? 2)"
                  class="w-full"
                  @input="setShadowOffsetX(Number(($event.target as HTMLInputElement).value))" />
              </div>
              <div>
                <label class="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Смещение Y</label>
                <input type="range" min="-20" max="20" step="1" :value="Number((selectedInstance.style as any)?.shadow?.offsetY ?? 2)"
                  class="w-full"
                  @input="setShadowOffsetY(Number(($event.target as HTMLInputElement).value))" />
              </div>
            </div>
          </div>
        </div>
